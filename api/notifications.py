...
AZURE_NOTIFICATION_HUB_CONNECTION_STRING = os.getenv(
...
class SendNotificationRequest(BaseModel):
    ...
    notification_type: str = Field(
        default="general", description="Type: transaction, request, security, general"
    )
...
async def send_notification_to_user(
    ...
    for device in devices:
        ...
        if USE_EXPO or platform == "expo":
            ...
            result = await send_azure_push_notification(
                push_token, platform, title, body, data
            )
...
async def register_push_token(
    request: RegisterPushTokenRequest, user: dict = Depends(get_current_user)
):
    """
    ...
    existing = next((d for d in devices if d["device_id"] == request.device_id), None)
...
async def unregister_push_token(device_id: str, user: dict = Depends(get_current_user)):
    """
    ...
    if user_token in _push_tokens:
        _push_tokens[user_token] = [
            d for d in _push_tokens[user_token] if d["device_id"] != device_id
        ]
...
async def update_notification_preferences(
    preferences: NotificationPreferences, user: dict = Depends(get_current_user)
):
    """
